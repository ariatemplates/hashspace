...
var $TextNode = klass({
	...
	/**
	 * Text node generator
	 * ex: Hello {person.name}!
	 * @param {Map<Expression>|int} exps the map of the variables used by the node. 
	 *   0 is passed if no  expression is used 
	 * @param {Array} textcfg array of the different text chunks that compose the text node
	 * 	 e.g. ["Hello ",0,"!"] odd elements are text fragments and even element are variable ids corresponding to t
	 */
	...
	getContent:function() {
		...
		if (this.isStatic) return tcfg[0]; // return fast on simple case
		else {
