 ...
 * @param path {Array} an array giving the path of the object. The first element can be undefined or can be
 *   the reference to the root path object in the original context - e.g. [lib,"lib","NbrField"]. If the first
 *   element is null or undefined, the path should be extracted from the scope object
 * @param scope {Object} the scope object from with the object should be extracted if the first path element is not defined
 */
...
/**
 * Component node
 * Allows to insert the content generated by a component template
 */
var $CptNode = klass({
	$extends:$RootNode,
	...
	/**
	 * $CptNode generator
	 * CptNode can be seen as a mix of InsertNode and EltNode
	...
	/**
	 * Create a node instance referencing the current node as base class
	 * As the $CptNode is DOMless it will not create a DOM node for itself - but will create
	 * nodes for its children instead (through the $RootNode of the template process function)
	 * @return {TNode} the new node instance
	 */
	createNodeInstance:function(parent) {
