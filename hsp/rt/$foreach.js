...
/**
 * foreach node
 * Implements the foreach conditional statement that can be used through 3 forms:
 * # foreach (itm in todos) // iteration over an array on the integer indexes - if todos in not an array "in" will be considered as "of"
 * # foreach (itm on todos) // same as Gecko object iteration - i.e. itm is the value of the item, not the key
 * # foreach (itm of todos) // same as Gecko object iteration (i.e. "on" mode), but with an extra  hasOwnProperty() validation - so items from the collection prototype will be ignored
 * => memorization trick: On = in for Objects / of is the special on with hasOwnProperty()
 *
...
var $ForEachNode = klass({
	 ...
	 * As the $ForEachNode is DOMless it will not create a DOM node for itself - but will create
	...
	/**
	 * Refresh the foreach nodes
	 * Check if the collection has changed - or i
	 */
	refresh:function() {
		if (this.adirty) {
	...
	/**
	 * Update the DOM of the displayed collection
	 * to match the target collection passed as argument
	...
	updateCollection:function(target) {
		...
		var itm, titm, idx, pendingItems=[], ditm, domIdx=0;
	 ...
	 * @param {Boolean} resyncNextItems resynchronize the scope variables (e.g. itm_key) of the next items - optional (default=true)
	 ...
	 * @param {Number} startIdx the index at which the search for nodes should start (optional - internal optimization - default: 0)
	 * @param {Boolean} resyncItems resynchronize the scope variables (e.g. itm_key) of the items - optional (default=true)
	 */
	moveItem:function(idx, newIdx, startIdx, resyncItems) {
		var cn=this.childNodes, sz=cn.length, ch;
		if (idx===newIdx || idx>sz-1 || newIdx>sz-1) return; // invalid cases
	 ...
	 * @param {Number} maxIdx the max index that should be considered to compute the itm_islast variable (optional - default: childNodes length)
	 */
...
var $ItemNode = klass({
	...
	/**
	 * ForEach Item node constructor
	 * This is a domless node that groups all the tnodes associated to a foreach item
	...
	detachDOMNodes:function(startIdx) {
		...
		if (this.detachedNodes) return; // already detached
		this.detachedNodes=[];
	...
	/**
	 * Insert the childNodes DOM nodes before a given node
	 * This assumes removeNodesFromDOM() has been called before
	 * @param {DOMNode} node the DOM node before which the
	 */
	attachDOMNodesBefore:function(node) {
		if (this.detachedNodes) {
			var dn=this.detachedNodes, sz=dn.length, nd;
